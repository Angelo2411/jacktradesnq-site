          ...
          <table className="v3-table">
            ...
            <tbody>
              {filteredStrats.map((s) => (
                <tr
                  key={s.slug}
                  onClick={() => { window.location.href = `/studies/${s.slug}/`; }}
                  style={{ cursor: 'pointer' }}
                >
                  <td>
                    <Link href={`/studies/${s.slug}/`} className="td-study" style={{ fontFamily: 'var(--f-serif)', fontWeight: 600, fontSize: '0.9rem' }}>
                      {s.event} IFVG+SMT
                    </Link>
                  </td>
                  <td>
          ...
          <table className="v3-table">
            ...
            <tbody>
              {filteredStudies.map((ms) => (
                <tr
                  key={ms.slug}
                  onClick={() => { window.location.href = `/studies/${ms.slug}/`; }}
                  style={{ cursor: 'pointer' }}
                >
                  <td>
                    <Link href={`/studies/${ms.slug}/`} style={{ fontFamily: 'var(--f-serif)', fontWeight: 600, fontSize: '0.9rem' }}>
                      {ms.title}
                    </Link>
                  </td>
                  <td>
