"""
Build ES event_bars JSONs for all 17 IFVG-SMT events.

Outputs 17 files → public/data/<short>_event_bars_es.json

Same shape as cpi_event_bars.json:
  [{date, t0_iso, entry_price, bars: [{m, o, h, l, c}, ...]}]

Run with: /Users/angelo/monfxreplay-python/.venv/bin/python scripts/build_event_bars_es.py
"""
...
ES_PARQUET = Path("/Users/angelo/winning-day-algo/data/ES_1m_10y_concat.parquet")
OUT_DIR = Path("/Users/angelo/jtnq-hub-v3-es/public/data")
...
# (event_type CSV, short slug, release_h, release_m) — all 17 events
EVENTS = [
    ("CPI",                  "cpi",                  8, 30),
    ("NFP",                  "nfp",                  8, 30),
    ("PPI",                  "ppi",                  8, 30),
    ("RetailSales",          "retailsales",          8, 30),
    ("PCE",                  "pce",                  8, 30),
    ("GDP",                  "gdp",                  8, 30),
    ("JoblessClaims",        "joblessclaims",        8, 30),
    ("EmpireState",          "empirestate",          8, 30),
    ("EmploymentCostIndex",  "employmentcostindex",  8, 30),
    ("FOMC",                 "fomc",                14,  0),
    ("ADP",                  "adp",                  8, 15),
    ("JOLTS",                "jolts",               10,  0),
    ("ISMMfg",               "ism_mfg",             10,  0),
...
def main():
    print(f"[ES] Connecting duckdb to {ES_PARQUET.name}...")
    con = duckdb.connect()
    con.execute(f"CREATE VIEW es AS SELECT ts, open, high, low, close FROM read_parquet('{ES_PARQUET}')")
    bounds = con.execute("SELECT min(ts), max(ts) FROM es").fetchone()
    ...
    for event_type, short, h, m in EVENTS:
        ...
        print(f"\n  [{short}_es] {len(dates)} events at {time_et}")
        ...
        for date_str in dates:
            ...
            df = con.execute(
                """
                SELECT ts, open, high, low, close FROM es
        ...
        out_path = OUT_DIR / f"{short}_event_bars_es.json"
