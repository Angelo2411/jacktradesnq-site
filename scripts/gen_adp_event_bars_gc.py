"""Generate ADP event_bars JSON for Gold (GC) — jtnq-hub-v3 calendar/trade overlay.

Output:
  ~/jtnq-hub-v3/public/data/adp_event_bars_gc.json

Pattern identique à gen_fomc_event_bars_gc.py adapté pour
ADP Non-Farm Employment Change à 08:15 ET.
"""
...
OUT_PATH = os.path.expanduser("~/jtnq-hub-v3/public/data/adp_event_bars_gc.json")
...
def load_adp_events() -> list[dict]:
    events = []
    with open(NEWS_CSV, newline="") as f:
        for row in csv.DictReader(f):
            if row["event_type"].strip() != "ADP":
                continue
            if row["time_et"].strip() != "08:15":
                continue
...
def build_t0_utc(date_str: str) -> datetime:
    ...
    local = naive.replace(hour=8, minute=15, tzinfo=ET)
...
def main():
    ...
    events = load_adp_events()
    print(f"[process] {len(events)} ADP events 08:15 ET", file=sys.stderr)
