"""Generate ISM Services event_bars JSON for Gold (GC) — jtnq-hub-v3 calendar/trade overlay.

Output:
  ~/jtnq-hub-v3/public/data/ism_services_event_bars_gc.json

Pattern identique à gen_adp_event_bars_gc.py adapté pour
ISM Services Index à 10:00 ET.
"""
...
OUT_PATH = os.path.expanduser("~/jtnq-hub-v3/public/data/ism_services_event_bars_gc.json")
...
def load_ism_services_events() -> list[dict]:
    events = []
    with open(NEWS_CSV, newline="") as f:
        for row in csv.DictReader(f):
            if row["event_type"].strip() != "ISMServices":
                continue
...
def main():
    ...
    events = load_ism_services_events()
    print(f"[process] {len(events)} ISMServices events 10:00 ET", file=sys.stderr)
