"""SI 10y 8:30 ET news IFVG runner — anchor=SI (silver), SMT partner=YM (E-mini Dow).

Loads SI + YM parquet ONCE for the window (2016-01-04 → 2024-12-31), then loops
...
Output per event (17 events × 2 files = 34 files + 1 combined = 35 total):
  - public/data/<event>-ifvg-smt-si-vs-ym.json          (rows + trades)
  - public/data/<event>-ifvg-smt-si-vs-ym-trade-prices.json  (raw prices)
  - public/data/si-ifvg-smt-vs-ym.json                  (combined, all 17 events)

SI-specific notes:
- Tick size: SI = 0.005 pt (Comex silver, full contract).
- SI parquet: /Users/angelo/backtesting/ict/data/metals/SI_1m_10y_real.parquet
  ts is datetime[ns, UTC]. 3.67M bars (sparse — missing minutes are not synthesized).
  Missing bar → no signal, no bias.
...
- SMT semantics: SI SHORT (swept UP) → YM low < YM pre_low;
                 SI LONG  (swept DOWN) → YM high > YM pre_high.
  Cross-asset divergence: metals swept in one direction while equity (Dow) confirms
  the opposite, suggesting the metals sweep is a liquidity grab not a real move.
- SL fix: full sweep displacement extreme between sweep_ts and entry_ts (inclusive)
  + 1 SI_TICK padding. Mirrors the fix applied in run_metals_10y_all_events.py for GC.

Usage:
    /Users/angelo/monfxreplay-python/.venv/bin/python run_si_with_ym_smt_10y.py 2>&1 | tee /tmp/si_ym_run.log
"""
...
# SI parquet starts 2016-01-03; YM parquet ends 2024-12-31.
DATE_FROM = "2016-01-04"
DATE_TO   = "2024-12-31"

# SI_TICK = 0.005 pt (Comex silver, full contract).
# simulate_variant and aggregate_rows are tick-agnostic: they operate on raw price pts.
SI_TICK = 0.005

TIMEOUT_SWEEP_HOUR_ET   = 11   # default sweep window end
...
EVENT_SLUG: dict[str, str] = {
    "CPI":                  "cpi-ifvg-smt-si-vs-ym",
    "NFP":                  "nfp-ifvg-smt-si-vs-ym",
    "PPI":                  "ppi-ifvg-smt-si-vs-ym",
    "RetailSales":          "retailsales-ifvg-smt-si-vs-ym",
    "PCE":                  "pce-ifvg-smt-si-vs-ym",
    "GDP":                  "gdp-ifvg-smt-si-vs-ym",
    "JoblessClaims":        "joblessclaims-ifvg-smt-si-vs-ym",
    "EmpireState":          "empirestate-ifvg-smt-si-vs-ym",
    "EmploymentCostIndex":  "employmentcostindex-ifvg-smt-si-vs-ym",
    "FOMC":                 "fomc-ifvg-smt-si-vs-ym",
    "ADP":                  "adp-ifvg-smt-si-vs-ym",
    "JOLTS":                "jolts-ifvg-smt-si-vs-ym",
    "ISMMfg":               "ism-mfg-ifvg-smt-si-vs-ym",
    "ISMServices":          "ism-services-ifvg-smt-si-vs-ym",
    "CBConfidence":         "cb-confidence-ifvg-smt-si-vs-ym",
    "PhillyFed":            "philly-fed-ifvg-smt-si-vs-ym",
    "DurableGoods":         "durable-goods-ifvg-smt-si-vs-ym",
}
...
def load_si_bars() -> dict[int, dict]:
    """Load SI real parquet → bar dict keyed by unix seconds (int).
    ts is datetime[ns, UTC]. Sparse — gaps are intentional.
    """
    path = "/Users/angelo/backtesting/ict/data/metals/SI_1m_10y_real.parquet"
    print(f"[load] SI 1m bars {DATE_FROM}..{DATE_TO}...", file=sys.stderr)
    df = pl.read_parquet(path)
    # ts is UTC — filter by epoch seconds
...
def load_ym_bars() -> dict[int, dict]:
    ...
    ts is datetime[ns, America/New_York]. Data ends 2024-12-31.
    """
    path = "/Users/angelo/backtesting/ict/data/YM_1m_10y.parquet"
...
# ---------------------------------------------------------------------------
# SI detect_setup (clone of detect_setup_metals with SI_TICK substituted)
# SL fix: full sweep displacement extreme between sweep_ts and entry_ts + 1 SI_TICK.
# ---------------------------------------------------------------------------

def detect_setup_si(ev: dict, bars_idx: dict[int, dict]) -> dict:
    """Sweep + IFVG entry detection for SI (tick = SI_TICK = 0.005).

    Structurally identical to detect_setup_metals in run_metals_10y_all_events.py.
    Cloned locally because TICK is hardcoded per-script.
    SL fix: full sweep displacement extreme between sweep_ts and entry_ts (inclusive)
    + 1 SI_TICK padding. Mirrors GC fix.
    """
    ...
    # Sweep watch starts at t0+60 (one bar after release)
    ...
    sl   = None  # computed after entry_ts known
    ...
    # Both-sides-swept invalidation (SI_TICK used here)
    opp_level_low  = pre_low  - SI_TICK   # used when sweep_dir == "UP"
    opp_level_high = pre_high + SI_TICK   # used when sweep_dir == "DOWN"
    ...
    # SL fix: full sweep displacement extreme between sweep_ts and entry_ts inclusive + 1 SI_TICK.
    sweep_disp_bars = [b for b in scan_bars if sweep_ts <= b["ts"] <= entry_ts]
    if not sweep_disp_bars:
        out["status"] = "SKIP_NO_SWEEP_DISP"
        return out
    if sweep_dir == "UP":
        sl = max(b["high"] for b in sweep_disp_bars) + SI_TICK
    else:
        sl = min(b["low"]  for b in sweep_disp_bars) - SI_TICK
...
# ---------------------------------------------------------------------------
# YM SMT detection (cross-asset: metals anchor, equity sister)
# SI SHORT (swept UP) → YM low < YM pre_low (Dow didn't sweep UP = divergence)
# SI LONG  (swept DOWN) → YM high > YM pre_high (Dow didn't sweep DOWN = divergence)
# ---------------------------------------------------------------------------

def compute_ym_smt(ev: dict, ym_idx: dict[int, dict], side: str) -> bool:
    """Did YM take its opposite-side pre-news liquidity during release→sweep_deadline window?

    SI SHORT (si swept UP) → ym low < ym pre_low (equity side confirms metals liquidity grab UP).
    SI LONG  (si swept DOWN) → ym high > ym pre_high.

    YM is dense equities data. Missing release bar → return False (no confirmation).
    """
    event_type = ev["event_type"]
    t0         = ev["t0_utc"]
    et_date    = ev["et_date"]
...
def collect_trade_db(event_type: str, si_idx: dict, ym_idx: dict) -> dict:
...
def main():
    ...
    print(f"[setup] loading SI + YM bars once ({DATE_FROM}..{DATE_TO})...", file=sys.stderr)
    si_idx = load_si_bars()
    ym_idx = load_ym_bars()
    ...
    for event_type in EVENTS:
        ...
        res = collect_trade_db(event_type, si_idx, ym_idx)
        ...
        summary_lines.append(
            ...
            f"net={r['net_pts']:+8.3f}  setups_ok={res['n_events_total']}/{res['n_csv_events']}"
        )

    # Sanity gate — SI is sparse so lower threshold than GC
    if len(all_trade_db) < 30:
        print(f"[ERROR] Only {len(all_trade_db)} raw trade records — check bar lookup!", file=sys.stderr)
        sys.exit(1)

    # Emit per-event JSONs into jtnq-hub-v3-ym public/data
    ...
    for event_type, evr in per_event_results.items():
        ...
        ev_out = {
            "meta": {
                "source": (
                    f"SI (Databento bbo-1m real, 2016-01-04…2024-12-31, {len(si_idx)} bars)"
                    f" + YM (SMT partner, equity sister, data ends 2024-12-31)"
                ),
                "date_from": DATE_FROM,
                "date_to":   DATE_TO,
                ...
                "smt": {
                    "false": "no SMT filter",
                    "true":  "YM took its target during release→sweep_deadline (SI short → YM low<pre_low; SI long → YM high>pre_high)",
                },
                "tick": "SI_TICK=0.005 (Comex silver full contract); detect_setup_si is a local clone of detect_setup_metals with SI_TICK substituted",
            },
            "rows":   evr["rows"],
            "trades": evr["trades"],
        }
        ...
        # Per-trade overlay file: <slug>-trade-prices.json
        prices_payload = []
        ...
        prices_out = {
            "generated_at": datetime.now(timezone.utc).isoformat(),
            "source":       "run_si_with_ym_smt_10y.py",
            "event":        event_type,
            "asset":        "SI",
    ...
    # Build combined rows + trades from all 17 events
    ...
    out = {
        "meta": {
            "source": (
                f"SI (Databento bbo-1m real, 2016-01-04…2024-12-31, {len(si_idx)} bars)"
                f" + YM (SMT partner, cross-asset metals↔equities)"
            ),
            "date_from": DATE_FROM,
            "date_to":   DATE_TO,
            ...
            "tick": "SI_TICK=0.005 (Comex silver full contract)",
        },
        "rows":   combined_rows,
        "trades": all_trades,
    }

    out_path = os.path.join(out_dir, "si-ifvg-smt-vs-ym.json")
    ...
    print(f"\n=== no_be / BOTH / ALL / smt=true (YM target) — SI 10y ALL 17 EVENTS ===")
    if r_headline:
        pf = r_headline["pf"] if r_headline["pf"] is not None else float("inf")
        pf_str = f"{pf:.2f}" if isinstance(pf, (int, float)) and pf != float("inf") else "inf"
        print(f"  n={r_headline['n']}  WR={int(round(r_headline['wr']*100))}%  PF={pf_str}  net={r_headline['net_pts']:+.3f} pts")
    print(f"\n=== per-event summary (smt=True, no_be, BOTH, ALL) ===")
