"""Generate FOMC event_bars JSON for Gold (GC) — jtnq-hub-v3 calendar/trade overlay.

Output:
  ~/jtnq-hub-v3/public/data/fomc_event_bars_gc.json

Pattern identique à gen_event_bars_gc.py (CPI/NFP @ 08:30 ET) adapté pour
FOMC statements à 14:00 ET.
"""
...
OUT_PATH = os.path.expanduser("~/jtnq-hub-v3/public/data/fomc_event_bars_gc.json")
...
def load_fomc_events() -> list[dict]:
    ...
    with open(NEWS_CSV, newline="") as f:
        for row in csv.DictReader(f):
            if row["event_type"].strip() != "FOMC":
                continue
            if row["time_et"].strip() != "14:00":
                continue
...
def build_t0_utc(date_str: str) -> datetime:
    ...
    local = naive.replace(hour=14, minute=0, tzinfo=ET)
...
def main():
    ...
    print(f"[process] {len(events)} FOMC events 14:00 ET", file=sys.stderr)
