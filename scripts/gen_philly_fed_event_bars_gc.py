"""Generate Philadelphia Fed event_bars JSON for Gold (GC) — jtnq-hub-v3 calendar/trade overlay.

Output:
  ~/jtnq-hub-v3/public/data/philly_fed_event_bars_gc.json

Pattern identique à gen_adp_event_bars_gc.py adapté pour
Philadelphia Fed Manufacturing Index à 08:30 ET.
"""
...
OUT_PATH = os.path.expanduser("~/jtnq-hub-v3/public/data/philly_fed_event_bars_gc.json")
...
def load_philly_fed_events() -> list[dict]:
    events = []
    with open(NEWS_CSV, newline="") as f:
        for row in csv.DictReader(f):
            if row["event_type"].strip() != "PhillyFed":
                continue
...
def main():
    ...
    events = load_philly_fed_events()
    print(f"[process] {len(events)} PhillyFed events 08:30 ET", file=sys.stderr)
