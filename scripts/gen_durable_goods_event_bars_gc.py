"""Generate Durable Goods Orders event_bars JSON for Gold (GC) — jtnq-hub-v3 calendar/trade overlay.

Output:
  ~/jtnq-hub-v3/public/data/durable_goods_event_bars_gc.json

Pattern identique à gen_adp_event_bars_gc.py adapté pour
Durable Goods Orders à 08:30 ET.
"""
...
OUT_PATH = os.path.expanduser("~/jtnq-hub-v3/public/data/durable_goods_event_bars_gc.json")
...
def load_durable_goods_events() -> list[dict]:
    events = []
    with open(NEWS_CSV, newline="") as f:
        for row in csv.DictReader(f):
            if row["event_type"].strip() != "DurableGoods":
                continue
            if row["time_et"].strip() != "08:30":
                continue
...
def build_t0_utc(date_str: str) -> datetime:
    ...
    local = naive.replace(hour=8, minute=30, tzinfo=ET)
...
def main():
    ...
    events = load_durable_goods_events()
    print(f"[process] {len(events)} DurableGoods events 08:30 ET", file=sys.stderr)
