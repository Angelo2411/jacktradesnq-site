"""ES 10y 8:30 ET news IFVG runner — YM SMT partner variant.

ES anchor + YM (DJIA futures) as SMT confirm partner.
17 events × 3 exit variants × 2-way YM SMT × 3 sides × 11 year buckets.

Loads ES + YM parquet ONCE for the full window (2016-01-04 → 2024-12-31), then loops
over 17 event types. Output schema matches the existing site Explorer.

Notes:
- Tick size: ES = 0.25 pt. simulate_variant and aggregate_rows reused unmodified.
- ES parquet: winning-day-algo continuous ~3.5M bars, ts in America/New_York ns.
- YM parquet (sister): /Users/angelo/backtesting/ict/data/YM_1m_10y.parquet
  (symlink → YM_1m_9y.parquet), ts in America/New_York tz, data ends 2024-12-31.
- DATE_TO clamped to 2024-12-31 (YM data stop).
- SMT semantics (same as NQ-partner): ES SHORT (sweep UP) → YM low < YM pre_low;
  ES LONG (sweep DOWN) → YM high > YM pre_high.
- Output paths: <event>-ifvg-smt-es-vs-ym.json + <event>-ifvg-smt-es-vs-ym-trade-prices.json
  + combined es-ifvg-smt-vs-ym.json

Usage:
    /Users/angelo/monfxreplay-python/.venv/bin/python run_es_with_ym_smt_10y.py
"""
...
EVENT_SLUG: dict[str, str] = {
    "CPI":                  "cpi-ifvg-smt-es-vs-ym",
    "NFP":                  "nfp-ifvg-smt-es-vs-ym",
    "PPI":                  "ppi-ifvg-smt-es-vs-ym",
    "RetailSales":          "retailsales-ifvg-smt-es-vs-ym",
    "PCE":                  "pce-ifvg-smt-es-vs-ym",
    "GDP":                  "gdp-ifvg-smt-es-vs-ym",
    "JoblessClaims":        "joblessclaims-ifvg-smt-es-vs-ym",
    "EmpireState":          "empirestate-ifvg-smt-es-vs-ym",
    "EmploymentCostIndex":  "employmentcostindex-ifvg-smt-es-vs-ym",
    "FOMC":                 "fomc-ifvg-smt-es-vs-ym",
    "ADP":                  "adp-ifvg-smt-es-vs-ym",
    "JOLTS":                "jolts-ifvg-smt-es-vs-ym",
    "ISMMfg":               "ism-mfg-ifvg-smt-es-vs-ym",
    "ISMServices":          "ism-services-ifvg-smt-es-vs-ym",
    "CBConfidence":         "cb-confidence-ifvg-smt-es-vs-ym",
    "PhillyFed":            "philly-fed-ifvg-smt-es-vs-ym",
    "DurableGoods":         "durable-goods-ifvg-smt-es-vs-ym",
}
...
# ---------------------------------------------------------------------------
# YM SMT detection (sister index for ES, replaces NQ)
# ---------------------------------------------------------------------------

def compute_ym_smt(ev: dict, ym_idx: dict[int, dict], side: str) -> bool:
    """Did YM take its target during release→sweep_deadline window?
    ES SHORT (ES swept UP) → YM low < YM pre_low.
    ES LONG  (ES swept DOWN) → YM high > YM pre_high.
    """
    event_type = ev["event_type"]
    t0 = ev["t0_utc"]
    et_date = ev["et_date"]
...
def collect_trade_db(event_type: str, es_idx: dict, ym_idx: dict) -> dict:
...
def main():
    ...
    print(f"[setup] loading ES + YM bars once ({DATE_FROM}..{DATE_TO})...", file=sys.stderr)
    es_idx = load_es_bars()
    ym_idx = load_ym_bars()
    ...
    for event_type in EVENTS:
        ...
        res = collect_trade_db(event_type, es_idx, ym_idx)
    ...
    for event_type, evr in per_event_results.items():
        ...
        ev_out = {
            "meta": {
                "source": (
                    "ES (S&P 500 futures, winning-day-algo, 2016-01-04…2024-12-31)"
                    " + YM (DJIA futures, SMT partner)"
                ),
                "date_from": DATE_FROM,
                "date_to":   DATE_TO,
                ...
                "smt": {
                    "false": "no SMT filter",
                    "true":  "YM took its OPPOSITE-side pre-news liquidity (ES short → YM low<pre_low; ES long → YM high>pre_high)",
                },
        ...
        prices_out = {
            "generated_at": datetime.now(timezone.utc).isoformat(),
            "source":       "run_es_with_ym_smt_10y.py",
            "event":        event_type,
            "asset":        "ES",
            "smt_partner":  "YM",
    ...
    out_path = "/Users/angelo/jtnq-hub-v3-ym/public/data/es-ifvg-smt-vs-ym.json"
    ...
    print(f"\n=== no_be / BOTH / ALL / smt=true (target) — ES 10y ALL 17 EVENTS (YM SMT) ===")
    if r_headline:
