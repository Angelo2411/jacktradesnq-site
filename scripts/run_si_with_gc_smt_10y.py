"""SI 10y 8:30 ET news IFVG runner — GC SMT partner variant.

SI anchor + GC (gold futures) as SMT confirm partner.
17 events × 3 exit variants × 2-way GC SMT × 3 sides × 11 year buckets.

Loads SI + GC parquet ONCE for the full 10y window (2016-01-04 → 2026-05-11), then loops
over 17 event types. Output schema matches the existing site Explorer.

Notes:
- Tick size: SI = 0.005 pt. simulate_variant and aggregate_rows reused unmodified.
- SI parquet: real Databento bbo-1m 3.68M bars, ts in UTC ns (same format as GC).
- GC parquet: forward-filled continuous 5.44M bars, ts in UTC ns.
- SMT semantics: SI SHORT (sweep UP) → GC low < GC pre_low;
  SI LONG (sweep DOWN) → GC high > GC pre_high.
- Output paths: <event>-ifvg-smt-si.json + <event>-ifvg-smt-si-trade-prices.json
  + combined si-ifvg-smt.json

Usage:
    /Users/angelo/monfxreplay-python/.venv/bin/python run_si_with_gc_smt_10y.py
"""
...
# SI tick = 0.005 pt (Comex silver). simulate_variant and aggregate_rows are tick-agnostic.
SI_TICK = 0.005
...
# Mapping event_type → site slug for SI anchor files
EVENT_SLUG: dict[str, str] = {
    "CPI":                  "cpi-ifvg-smt-si",
    "NFP":                  "nfp-ifvg-smt-si",
    "PPI":                  "ppi-ifvg-smt-si",
    "RetailSales":          "retailsales-ifvg-smt-si",
    "PCE":                  "pce-ifvg-smt-si",
    "GDP":                  "gdp-ifvg-smt-si",
    "JoblessClaims":        "joblessclaims-ifvg-smt-si",
    "EmpireState":          "empirestate-ifvg-smt-si",
    "EmploymentCostIndex":  "employmentcostindex-ifvg-smt-si",
    "FOMC":                 "fomc-ifvg-smt-si",
    "ADP":                  "adp-ifvg-smt-si",
    "JOLTS":                "jolts-ifvg-smt-si",
    "ISMMfg":               "ism-mfg-ifvg-smt-si",
    "ISMServices":          "ism-services-ifvg-smt-si",
    "CBConfidence":         "cb-confidence-ifvg-smt-si",
    "PhillyFed":            "philly-fed-ifvg-smt-si",
    "DurableGoods":         "durable-goods-ifvg-smt-si",
}
...
def load_si_bars() -> dict[int, dict]:
    """Load SI real Databento parquet → bar dict keyed by unix seconds (int).
    ts column is datetime[ns, UTC] — same format as GC ffill parquet.
    """
...
def load_gc_bars() -> dict[int, dict]:
    ...
    Used as SMT partner (sister). ts is datetime[ns, UTC].
    """
    path = "/Users/angelo/backtesting/ict/data/metals/GC_1m_10y_ffill.parquet"
    print(f"[load] GC 1m bars {DATE_FROM}..{DATE_TO} (SMT partner)...", file=sys.stderr)
    df = pl.read_parquet(path)
    from_ts = int(datetime.strptime(DATE_FROM, "%Y-%m-%d").replace(tzinfo=timezone.utc).timestamp())
    to_ts   = int(datetime.strptime(DATE_TO,   "%Y-%m-%d").replace(tzinfo=timezone.utc).timestamp()) + 86400
    df = df.filter(
    ...
    print(f"[load] GC {len(bars)} bars indexed (SMT partner)", file=sys.stderr)
    return bars
...
# ---------------------------------------------------------------------------
# SI-specific detect_setup (clone of detect_setup_metals with SI_TICK)
# ---------------------------------------------------------------------------
...
def detect_setup_si(ev: dict, bars_idx: dict[int, dict]) -> dict:
    ...
    Cloned locally because TICK is hardcoded at module level in that file as 0.10 (GC).
    Preserves the SL sweep-displacement fix: SL = worst extreme between sweep_ts and
    entry_ts (inclusive) + 1 SI_TICK padding.
    """
    ...
    # Sweep watch starts at t0+1 (release+1min), AFTER the release bar closes
    ...
    # SL = full sweep displacement extreme (between sweep_ts and entry_ts inclusive) + 1 SI_TICK padding.
...
# ---------------------------------------------------------------------------
# GC SMT detection (SI anchor, GC sister)
# SI SHORT (SI swept UP) → GC low < GC pre_low
# SI LONG  (SI swept DOWN) → GC high > GC pre_high
# ---------------------------------------------------------------------------

def compute_gc_smt(ev: dict, gc_idx: dict[int, dict], side: str) -> bool:
    """Did GC take its target during release→sweep_deadline window?
    SI SHORT (SI swept UP) → GC low < GC pre_low.
    SI LONG  (SI swept DOWN) → GC high > GC pre_high.
    GC is dense (ffill): missing release bar returns False (conservative, no noise).
    """
    event_type = ev["event_type"]
    t0 = ev["t0_utc"]
    et_date = ev["et_date"]
    ...
    gc_release = gc_idx.get(t0)
    if gc_release is None:
        return False
    gc_pre_high = gc_release["high"]
    gc_pre_low  = gc_release["low"]
    ...
    if side == "SHORT":
        # SI swept UP → expect GC to also take its LOW (divergence = GC DIDN'T take low)
        return gc_low_swept
    else:
        # SI swept DOWN → expect GC to also take its HIGH (divergence = GC DIDN'T take high)
        return gc_high_swept
...
def collect_trade_db(event_type: str, si_idx: dict, gc_idx: dict) -> dict:
    """Run engine for one event_type. SI is anchor, GC is SMT partner."""
    ...
    for ev in events:
        ...
        if s["status"] != "OK":
            continue
        n_ok += 1
        smt_target = compute_gc_smt(ev, gc_idx, s["side"])
...
def main():
    ...
    print(f"[setup] loading SI (anchor) + GC (SMT partner) bars once ({DATE_FROM}..{DATE_TO})...", file=sys.stderr)
    si_idx = load_si_bars()
    gc_idx = load_gc_bars()
    ...
    for event_type in EVENTS:
        ...
        res = collect_trade_db(event_type, si_idx, gc_idx)
    ...
    for event_type, evr in per_event_results.items():
        ...
        ev_out = {
            "meta": {
                "source": (
                    "SI.n.0 (Databento bbo-1m real, 10y, 2016-01-04…2026-05-11)"
                    " + GC.n.0 (SMT partner, forward-filled)"
                ),
                "date_from": DATE_FROM,
                "date_to":   DATE_TO,
                ...
                "smt": {
                    "false": "no SMT filter",
                    "true":  "GC took its OPPOSITE-side pre-news liquidity (SI short → GC low<GC pre_low; SI long → GC high>GC pre_high)",
                },
                "tick": "SI_TICK=0.005 (Comex silver); detect_setup_si is a local clone of detect_setup_metals with SI_TICK substitution",
            },
            "rows":   evr["rows"],
            "trades": evr["trades"],
        }
        # Output: <slug>.json (e.g. cpi-ifvg-smt-si.json)
        ...
        prices_out = {
            "generated_at": datetime.now(timezone.utc).isoformat(),
            "source":       "run_si_with_gc_smt_10y.py",
            "event":        event_type,
            "asset":        "SI",
    ...
    out_path = "/Users/angelo/jtnq-hub-v3/public/data/si-ifvg-smt.json"
    ...
    print(f"\n=== no_be / BOTH / ALL / smt=true (target) — SI 10y ALL 17 EVENTS ===")
    if r_headline:
